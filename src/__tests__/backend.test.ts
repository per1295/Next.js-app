...
describe("/blog", () => {
    ...
    test("check getPath", () => {
        ...
        expect(mockGetPath.mock.results.at(0)?.value).toBe(`/api/blog/${arrayOfAdditionalPaths.at(0)}`);
        expect(mockGetPath.mock.results.at(1)?.value).toBe(`/api/blog/${arrayOfAdditionalPaths.at(1)}`);
        expect(mockGetPath.mock.results.at(2)?.value).toBe(`/api/blog/${arrayOfAdditionalPaths.at(2)}`);
    });
...
describe("/contact", () => {
    ...
    test("POST /user (fake user)", async () => {
        const testData = {
            name: "Mike",
        ...
        const responseData = {
            message: "This user needs verification",
            status: "success",
        }
